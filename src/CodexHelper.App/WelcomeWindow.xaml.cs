...
public partial class WelcomeWindow : Window
{
    ...
    [
        ...
        new("Collaboration", "协作开发：GPT 规划、执行器编码", "协作开发独立于连接中心，集中管理 GPT + 实现执行器（Reasonix / DeepSeek Harness）的协作设置。", "先在上方选择实现执行器。选择 Reasonix 时，点“开启协作编码”安装 Reasonix 执行 Skill，随后可按需设置默认模型、权限模式和执行强度，点“测试连接”确认可用。选择 DeepSeek Harness 时，先“重新检测”确认 Node/dsh/Web Host 环境，再点“开启协作编码”应用协作规则；可设置执行模式（默认 Codex 合同模式）、权限（默认完全控制）与执行强度，并可用“应用并测试”验证。", "最近任务可在这里刷新、停止或重试；首次启用或关闭协作后需重新打开 Codex。"),
